...
# 🆕 Выносим промт в отдельную функцию/переменную для переиспользования
def get_math_prompt(user_text):
    """Возвращает форматированный промт для математических запросов."""
    return f"""
...
def web_data(m):
    ...
    h = get_history(cid)
    # 💥 ИСПОЛЬЗУЕМ НОВУЮ ФУНКЦИЮ ПРОМПТА
    h.append({"role": "user", "content": get_math_prompt(full_request)})
...
def txt(m):
    ...
    # 💥 ВАЖНОЕ ИЗМЕНЕНИЕ: Вставляем промт с правилами форматирования
    h.append({"role": "user", "content": get_math_prompt(m.text)})
